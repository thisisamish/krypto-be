...
import com.groupeight.krypto.dto.CartItemResponseDto;
...
import com.groupeight.krypto.repository.CartItemRepository;
...
public class CartServiceImpl implements CartService {
	...
	public CartResponseDto addItem(Long userId, Long productId, int quantity) {
		...
		CartItem item = cart.getItems().stream().filter(ci -> ci.getProduct().getId().equals(productId)).findFirst()
				.orElse(null);
	...
	private CartResponseDto toDto(Cart cart) {
		List<CartItemResponseDto> items = cart.getItems().stream()
				...
				.map(ci -> CartItemResponseDto.builder().productId(ci.getProduct().getId())
						.productName(ci.getProduct().getName()).unitPrice(ci.getUnitPrice()).quantity(ci.getQuantity())
